...
TEST(GameTest, calculateDirection)
{
    Game game;
    sf::Vector2f position(100, 100);
    float speed = 50;
    sf::Vector2f velocity = game.calculate_direction(position);
    ...
    sf::Vector2f expected_velocity = direction;
...
TEST(GameTest, playerMove)
{
    ...
    sf::Vector2f velocity = 0.016f * player_speed * game.calculate_direction(position_before_move);
...
TEST(GameTest, splitBalls)
{
    ...
    GTEST_ASSERT_EQ(after_split, 2 * before_split);
}
